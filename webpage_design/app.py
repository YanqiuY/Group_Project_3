...
app.config['SQLALCHEMY_DATABASE_URI'] = "sqlite:///AQI.sqlite"
db = SQLAlchemy(app)
...
@app.route("/api/aqi")
def get_aqi():    
    results = session.query(AQI.AQIValue, func.count(AQI.AQIValue)).\
    group_by(AQI.AQIValue).order_by(func.count(AQI.AQIValue).desc()).all()
